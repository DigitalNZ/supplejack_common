...
describe SupplejackCommon::FileResource do
  ...
  describe '#attributes' do
    %i[size height width mime_type extension url].each do |attribute|
      it "should populate the #{attribute} field" do
        allow(subject).to receive(attribute) { 'Hi' }
  ...
  describe '#extension' do
    it 'returns the file extension by inspecting the path' do
      allow(subject).to receive(:url) { 'http://google.com/image.jpg' }
      expect(subject.extension).to eq 'jpg'
    end

    it 'downcases the extension' do
      allow(subject).to receive(:url) { 'http://google.com/image.PNG' }
      expect(subject.extension).to eq 'png'
    end

    it 'falls back on the mime type when the extension is not reliable' do
      allow(subject).to receive(:url) { 'http://muse.aucklandmuseum.com/databases/images.aspx?FileName=F%3a%5cData%5cDBIMAGES%5cMAPS%5cf%5cG9230-1978.jpg&filepathtype=unc&width=240&height=600&cmd=scaledown' }
      mime_content = allow(MimeMagic).to receive(:by_magic) { double(:mime, extensions: %w[jpe jpeg jpg]) }
      allow(MimeMagic).to receive(:by_path) { nil }
