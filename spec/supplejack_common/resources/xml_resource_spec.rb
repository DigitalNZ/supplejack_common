...
describe SupplejackCommon::XmlResource do
  ...
  describe '#initialize' do
    it 'sets the namespaces class attribute' do
      expect(subject.class._namespaces[:dc]).to eq 'http://purl.org/dc/elements/1.1/'
    end
  end

  describe '#document' do
    it 'parses the resource as XML' do
      allow(subject).to receive(:fetch_document).and_return('</xml>')
  ...
  describe '#strategy_value' do
    ...
    it 'creates a new XpathOption with the namespaces class attribute' do
