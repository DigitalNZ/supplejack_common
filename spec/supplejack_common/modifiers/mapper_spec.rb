...
describe SupplejackCommon::Modifiers::Mapper do
  ...
  describe 'modify' do
    ...
    it 'makes multiple modifications' do
      allow(subject).to receive(:replacement_rules).and_return({ /width=[\d]{1,4}/ => 'width=520', /height=[\d]{1,4}/ => 'height=310' })
    ...
    it 'returns the original value when it does not match any rule' do
      allow(subject).to receive(:replacement_rules).and_return({ /microsoft=[\d]/ => 'anything' })
