...
describe HarvesterCore::Xml::Base do
  ...
  before do
    urls_xml = File.read("spec/harvester_core/integrations/source_data/xml_parser_urls.xml")
    stub_request(:get, "http://www.nzonscreen.com/api/title/").to_return(:status => 200, :body => urls_xml)

    record_xml = File.read("spec/harvester_core/integrations/source_data/xml_parser_record.xml")
