...
describe HarvesterCore::PaginatedCollection do
  
  let(:klass) { HarvesterCore::PaginatedCollection }
  let(:collection) { klass.new(HarvesterCore::Base, {page_parameter: "page", type: "item", per_page_parameter: "per_page", per_page: 5, page: 1}, {limit: 1}) }

  
  describe "#initialize" do
    it "assigns the klass" do
      collection.klass.should eq HarvesterCore::Base
    end
  ...
  describe "#each" do
    ...
    it "should process all base_urls" do
      HarvesterCore::Base.should_receive(:fetch_records).with("http://go.gle/")
      HarvesterCore::Base.should_receive(:fetch_records).with("http://dnz.harvest/1")
    ...
    context "paginated" do
      ...
      before do 
        ...
        collection.klass.stub(:base_urls) { ["http://go.gle/", "http://dnz.harvest/1"]}
        HarvesterCore::Base.stub(:_total_results) { 1 }
      end

      it "should call fetch records with a paginated url" do
        HarvesterCore::Base.should_receive(:fetch_records).with("http://go.gle/?page=1&per_page=10")
        HarvesterCore::Base.should_receive(:fetch_records).with("http://dnz.harvest/1?page=1&per_page=10")
        collection.each do; end
      end
    end
  end
  ...
  describe "#paginated?" do
    ...
    it "returns false when no pagination options are set" do
      collection = klass.new(HarvesterCore::Base, nil, {})
  ...
  describe "#current_page" do
    context "page type pagination" do
      let(:collection) { klass.new(HarvesterCore::Base, {page_parameter: "page", type: "page", per_page_parameter: "per_page", per_page: 5, page: 1}) }
    ...
    context "item type pagination" do
      let(:collection) { klass.new(HarvesterCore::Base, {page_parameter: "page", type: "item", per_page_parameter: "per_page", per_page: 5, page: 1}) }
