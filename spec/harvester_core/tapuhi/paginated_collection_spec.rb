...
describe HarvesterCore::Tapuhi::PaginatedCollection do

  class TestSourceTapuhi < HarvesterCore::Tapuhi::Base
    run_length_bytes 8
  end

  let(:klass) { TestSourceTapuhi }
  let(:record) { mock(:record).as_null_object }
  let(:collection) { HarvesterCore::Tapuhi::PaginatedCollection.new(TestSourceTapuhi, ["/path/to/file"], "10") }
